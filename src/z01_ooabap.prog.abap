...
CLASS lcl_z01_po DEFINITION.
  PUBLIC SECTION.
    ...
    INTERFACES lif_z01_details.
ENDCLASS.


CLASS lcl_z01_po IMPLEMENTATION.


  METHOD lif_z01_details~get_details .
    SELECT SINGLE ebeln, bsart, statu, lifnr FROM ekko WHERE ebeln = @i_pono
    INTO CORRESPONDING FIELDS OF @e_ekko.

    IF sy-subrc <> 0.
      lv_symsgv = i_pono.
      RAISE EXCEPTION TYPE zcx_01_exception_class
        EXPORTING
*         textid = lw_msg
          msgid  = 'Z01_MSGCLASS'
          msg1   = lv_symsgv
  ...
  METHOD lif_z01_details~display_details.
...
CLASS lcl_z01_so IMPLEMENTATION.
...
*--Instantiate the first class--*
  DATA(lo_object) = NEW lcl_z01_po( ).
  DATA: lo_msg TYPE REF TO zcx_01_exception_class,
        lo_msg_1 TYPE REF TO zcx_01_exception_class.

*--Instantiate the second class--*
  DATA(lo_object_so) = NEW lcl_z01_so( ).

*--Call method to get Purchase Order details--*
  TRY.
      lo_object->lif_z01_details~get_details( EXPORTING
                                      i_pono =  lp_po
                                      i_sono = lp_so
                                   IMPORTING
      ...
      lo_object->lif_z01_details~display_details( EXPORTING
                                      e_ekko = lo_object->wa_po
                                      e_vbak = lo_object_so->wa_so ).
    CATCH zcx_01_exception_class INTO lo_msg.
...
*--Call method to get sales Order details--*
  TRY.
      lo_object_so->lif_z01_details~get_details( EXPORTING
                                      i_pono =  lp_po
                                      i_sono = lp_so
                                   IMPORTING
                                      e_vbak = lo_object_so->wa_so ).
      ...
      " calling display function
      lo_object_so->lif_z01_details~display_details( EXPORTING
                                      e_ekko = lo_object->wa_po
                                      e_vbak = lo_object_so->wa_so ).
    CATCH zcx_01_exception_class INTO lo_msg.
